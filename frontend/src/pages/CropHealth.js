...
const CropHealth = () => {
  ...
  React.useEffect(() => { setAnalysis(null); }, [farmId]);
      ...
      <div className="bg-gradient-to-br from-green-600 to-green-500 text-white p-6 md:p-8">
        <div className="max-w-7xl mx-auto flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-white/90">Rule-based assessment from sensor readings</p>
          </div>
          <FarmSwitcher light />
        </div>
      </div>
            ...
            <Card className="border-2">
              ...
              <CardContent>
                <div className="flex flex-wrap items-center gap-3 text-muted-foreground text-sm">
                  <span className="flex items-center gap-1"><TrendingUp className="w-4 h-4" />Confidence: {Math.round(analysis.confidence * 100)}%</span>
                  <span>· Method: {analysis.method}</span>
                  <SourceBadge source={analysis.data_source} />
                </div>
              </CardContent>
            </Card>
